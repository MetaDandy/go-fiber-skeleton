...
func setupTestContainer(t *testing.T) *gorm.DB {
	...
	dbOnce.Do(func() {
		...
		// Run seed for permissions (19 permissions)
		seed.SeedPermissions(testDB)
	})

	// Truncate tables before each test for isolation
	// This runs for EACH test that calls setupTestContainer
	t.Cleanup(func() {
		if testDB != nil {
...
func truncateTables(t *testing.T, db *gorm.DB) {
	...
	// Explicitly list tables to truncate (in order to handle FK constraints)
	// Skip: schema_migrations (goose tracking), permissions (seeded data)
	tables := []string{
		"userpermissions",
		"sessions",
		"auth_logs",
...
func TestSetupTestContainer_CanQueryUserPermissions(t *testing.T) {
	db := setupTestContainer(t)

	// Try to query userpermissions table (should exist after migrations)
	var count int64
	err := db.Raw("SELECT COUNT(*) FROM userpermissions").Scan(&count).Error
	assert.NoError(t, err, "failed to query userpermissions table")
}
